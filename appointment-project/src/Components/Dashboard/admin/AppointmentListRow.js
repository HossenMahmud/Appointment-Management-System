...
import { Avatar, Button, Grid, Modal, Paper, Table, TableBody, TableCell, tableCellClasses, TableContainer, TableRow, Typography } from '@mui/material';
import { Box } from '@mui/system';
import React, { useState } from 'react';
import VisibilityIcon from '@mui/icons-material/Visibility';
import { StatusBtnPendding, StatusBtnAccept, StatusBtnCancel } from '../../../styles/MetarialStyles';
...
const AppointmentListRow = ({ data, index }) => {
        ...
        <>
            <StyledTableRow key={index}>
                <StyledTableCell align="left" sx={{ display: 'flex', alignItems: 'center' }}>
                    <Avatar alt="Remy Sharp" src={data?.doctorImg} sx={{ mr: 1 }} />
                    <Box>
                        {data?.doctorName}
                        <Typography sx={{ color: 'blue' }}>{data?.specialist}</Typography>
                    </Box>
                </StyledTableCell>
                ...
                <StyledTableCell align="left">
                    {
                        (data.status === 'pendding') && <StatusBtnPendding> {data.status}</StatusBtnPendding>
                    }
                    {
                        (data.status === 'Accepted') && <StatusBtnAccept> {data.status}</StatusBtnAccept>
                    }
                    {
                        (data.status === 'Cancel') && <StatusBtnCancel> {data.status}</StatusBtnCancel>
                    }
                </StyledTableCell>
                <StyledTableCell align="left">
                                        ...
                                        <Table sx={{ maxWidth: { xs: '300', sm: '400' } }} aria-label="simple table" >
                                            <TableBody sx={{ '&:last-child td, &:last-child th': { border: '1px solid #f1f1f1' } }}>
                                                <TableRow >
                                                    ...
                                                    <TableCell align="right">Dr. {data?.doctorName}</TableCell>
                                                </TableRow>
                                                <TableRow >
                                                    <TableCell align="left">Specialist</TableCell>
                                                    <TableCell align="right">{data?.specialist}</TableCell>
                                                </TableRow>
                                                <TableRow >
